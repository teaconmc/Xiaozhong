...
import PackMCMeta from '!!raw-loader!./pack.mcmeta'
import ModsToml from '!!raw-loader!./mods.toml'
...
<CodeBlock language="json" title="src/main/resources/pack.mcmeta" showLineNumbers>{PackMCMeta}</CodeBlock>

:::tip

关于 `pack.mcmeta` 的具体格式，请参见 Minecraft Wiki 对[数据包](https://minecraft.fandom.com/zh/wiki/%E6%95%B0%E6%8D%AE%E5%8C%85)和[资源包](https://minecraft.fandom.com/zh/wiki/%E8%B5%84%E6%BA%90%E5%8C%85)的介绍。`pack_format` 指定了数据包和资源包的版本，对于 Minecraft 1.19.4，Forge 推荐使用 `12` 作为 `pack_format` 的值。

可能有曾开发过之前版本的 Mod 的读者已经发现了，与之前相比多出了两个 key： `forge:resource_pack_format` 与 `forge:data_pack_format`，这两个是 forge 对原版 pack.mcmeta 的修改，允许 resource_pack 与 data_pack 使用不同的版本。
...
<CodeBlock language="toml" title="src/main/resources/META-INF/mods.toml" showLineNumbers>{ModsToml}</CodeBlock>
...
<TabItem value="full" label="完整代码">
    <CodeBlock language="java" metastring="{14,17-22}" showLineNumbers
...
```java title="src/main/java/org/teacon/xiaozhong/Xiaozhong.java"
// 为监听器的外部类添加 @Mod.EventBusSubscriber 注解
...
<TabItem value="full" label="完整代码">
    <CodeBlock language="java" metastring="{15-22}" showLineNumbers
...
如果使用 `Component.translatable`，则游戏会去寻找[语言文件](https://minecraft.fandom.com/zh/wiki/%E8%B5%84%E6%BA%90%E5%8C%85#.E8.AF.AD.E8.A8.80)中的翻译标识符并按照语言文件的规则翻译。如果翻译失败则游戏将直接显示翻译标识符本身。模组开发者也可以指定自己的语言文件，并放在 `[模组 ID]:[语言代码]` 亦即 `assets/[模组 ID]/lang/[语言代码].json` 处，常见的语言代码有 `en_us` 及 `zh_cn` 等。语言文件亦可使用 Data Generator 生成。
...
<TabItem value="full" label="完整代码">
    <CodeBlock language="java" metastring="{16-17,20-23,25-29,31-40,42-51}" showLineNumbers
