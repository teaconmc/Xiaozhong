...
资源路径在源代码中为 `Identifier`，如 `Identifier.fromNamespaceAndPath("foo", "bar")` 即代表 `foo:bar` 这一资源路径。

命名空间如为 `minecraft` 则可使用 `Identifier.withDefaultNamespace("bar")`，代表 `minecraft:bar`。
